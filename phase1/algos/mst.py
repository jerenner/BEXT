...
class mst(AAlgo):
    ...
    def execute(self,event=""):
        ...
        if(self.use_voxels):
            hList = self.event.GetHits(gate.SIPM)
        ...
        # Determine the number of voxels in each segment.
        fsegments.sort(key = lambda x: len(x.path),reverse=True)
        mst_seglens = std.vector(int)()
        for sseg in fsegments:
            mst_seglens.push_back(len(sseg.path))
        main_trk.store("MSTSegLens",mst_seglens)
        ...
        if(self.plt_tracks):
            ...
            ax1.set_ylabel("y (mm)");
            ax1.set_zlabel("z (mm)");
            ax1.set_title("Smeared Monte Carlo Truth");
    ...
    def trk_mst(self,hList):
        ...
        for nA in trknode_list:
            ...
            for nB in trknode_list:
                ...
                if(ndist > 0):
                    if(debug > 2): print "Adding edge from {0} to {1} with weighting of {2}".format(nA.id,nB.id,ndist + 0.001*(1.0*nA.orderid/ntot_nodes + 1.0*nB.orderid/ntot_nodes**2))
    #                trk.add_edge(nA,nB,weight=(ndist+0.000001*nA.id+0.000001*nB.id));
        ...
        for seg in segments:
            ...
            ep1 = seg.inode;
            seg1 = self.match_to_segments(ep1,seg,segments_f,trk,adjMat);
            if(seg1 != 0):
                if(debug > 1): print "Appending segment of {0} nodes: {1}".format(len(seg1.path),seg1);
                segments_f.append(seg1);
                
            ep2 = seg.fnode;
            seg2 = self.match_to_segments(ep2,seg,segments_f,trk,adjMat);
            if(seg2 != 0): 
    ...
    def match_to_segments(self,node,nseg,segments,gtrk,adjMat):
        ...
        for seg in segments:
            ...
            pth1 = nx.astar_path(gtrk,node,ep1);
            pth2 = nx.astar_path(gtrk,node,ep2);
            ...
            if(not ovr1 and not ovr2):
                ...
                seg1.set_path(pth1);
                seg1.set_len(self.path_length(gtrk,adjMat,pth1));
                return seg1;
            elif(not ovr2):
                ...
                seg2.set_path(pth2);
                seg2.set_len(self.path_length(gtrk,adjMat,pth2));
                return seg2;
